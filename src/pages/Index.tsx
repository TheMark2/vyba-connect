...
const Index = () => {
  ...
  const artists = [{
    ...
    type: "Violinista",
    image: "https://images.unsplash.com/photo-1465821185615-20b3c2fbf41b?w=1200&h=800&auto=format&fit=crop",
    description: "Descubre violinistas para tu ocasión especial"
  }];
  ...
  const searchPlaceholder = useTransform(scrollYProgress, [0, 0.25, 0.5, 0.75], ["Buscar artistas", "Buscar saxofonistas", "Buscar guitarristas", "Buscar violinistas"]);
  const headingText = useTransform(scrollYProgress, [0, 0.25, 0.5, 0.75], ["El portal perfecto para encontrar tu dj", "Descubre saxofonistas profesionales", "Conecta con guitarristas talentosos", "Encuentra violinistas para tu evento"]);
  
  // Adding a left padding transform that increases with scroll
  const leftPadding = useTransform(scrollYProgress, [0, 1], [0, 100]);
              ...
              <motion.div className="absolute inset-0 px-6 md:px-10 lg:px-14 xl:px-16 pt-8 pb-32">
                <motion.div className="relative w-full h-full rounded-[2vw] overflow-hidden">
                  ...
                  <motion.div className="absolute inset-0" style={{
                    ...
                    <img src={artists[3].image} alt="Violinista performing" className="w-full h-full brightness-75 object-cover" />
                  </motion.div>
