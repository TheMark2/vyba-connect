...
const ReviewItem = ({
  ...
  return <div className="pb-8 border-b border-gray-200 dark:border-gray-700">
      {isMobile ?
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-6 mb-8 justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            {genres?.filter((_, i) => i < 2).map((genre, index) => <Badge key={index} variant="outline" className="py-2 px-4 bg-white border-0 text-sm font-medium flex items-center gap-2 dark:bg-vyba-dark-secondary">
                {genre}
              </Badge>)}
          </div>
        </div>
        ...
        <div className="space-y-10">
          {reviewsData?.slice(0, 3).map(review => <ReviewItem key={review.id} review={review} />)}
        </div>
